...
int main(int argc, char* argv[]) {
    ...
    pthread_t threadprod[prodnum];
    pthread_t threadcons[consnum];

    sem_init(&pode_c, 0, 0);
    sem_init(&pode_p, 0, prodnum);
    
    for(long i = 0; i < prodnum; i++){
        printf("Thread produtora %d criada\n", (int) i);
        pthread_create(&threadprod[i], NULL, &produtor, (void *) i);
    }
    printf("boas\n");

    for(long i = 0; i < consnum; i++){
        printf("Thread consumidora %d criada\n",(int) i);
        pthread_create(&threadcons[i], NULL, &consumidor, (void *) i);
    }

    for(int k = 0; k < prodnum; k++){
        printf("Thread produtora %d terminada\n", k);
        pthread_join(threadprod[k], NULL);
    }


    for(int k = 0; k < consnum; k++){
        pthread_join(threadcons[k]);
    }
    get_time_useconds(&time_usec_end);
    elapsed_time = (long) (time_usec_end - time_usec_begin);
...
void * produtor (void * param){
    ...
    while(g != NULL){
        ...
        MUTATION_ARRAY * item = produzir(g);
        sem_wait(&pode_p);
...
void * consumidor (void * param){
    ...
    sprintf(out, "result/mutation_prod_cons.txt");
    while(1){
        MUTATION_ARRAY * item;
        sem_wait(&pode_c);
        ...
        sem_post(&pode_p);
        save_mutation_array_thread(item, out, 0, pthread_self());
        free(item);
    }
    pthread_exit(NULL);
}
