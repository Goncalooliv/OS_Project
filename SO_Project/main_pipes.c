 ...
 */
    ...
    for(int i = 0; i < chldnum; i++){
        ...
        if(child[i] == 0){
            ...
            while (g != NULL) {
                ...
                pipe_write(mutation_array, fds, getpid(), 0);
    ...
    pipe_read("result/mutations_pipes.txt", fds);

    for(int i = 0; i < chldnum; i++){
        int result; 
        waitpid(child[i], &result, 0);
    }
...
/**
 * Os Filhos vão escrever as comparações para o pipe
 * @param mutation_array - array com todas as mutações
 * @param fds - pointer to pipe
 * @param chldpid - pid do filho
...
void pipe_write(MUTATION_ARRAY * mutation_array, int *fds, int chldpid, int detail){
    ...
    for (int i=0 ; i<mutation_array->n_mutations ; i++) {
        ...
        strcat(buf2, "\n");
        writen(fds[1], buf2, strlen(buf2)); //escrever para o pipe
    }
...
/**
 * Pai a ler do pipe e escrever do ficheiro
 * @param path - path para o ficheiro na qual os resultados vão ser guardados
...
void pipe_read(char *path, int *fds){
    ...
    while((text = readn(fds[0], bufr, BUF_SIZE)) > 0) {
        ...
        while(token !=NULL){
            ...
            token = strtok(NULL, "#"); //passamos NULL para dizer à função que é para continuar a segmentar o bufr
        }
    }
    close(fd);
    free(token);
